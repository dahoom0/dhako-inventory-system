...
export default function Sidebar({ current, onNav, expanded, onToggleExpand, open, onClose, onLogout }: Props) {
  ...
  return (
    <>
      ...
      <aside
        className="fixed md:static inset-y-0 left-0 z-30 flex flex-col transition-transform duration-200 md:translate-x-0"
        style={{ width: 220, background: "#1e3a8a", flexShrink: 0, transform: open ? "translateX(0)" : undefined }}
      >
        {/* Logo */}
        <div className="flex flex-col items-center py-5 px-4 gap-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.12)" }}>
          <div className="w-14 h-14 rounded-full overflow-hidden" style={{ border: "2px solid rgba(255,255,255,0.3)" }}>
            <img src={dhakoLogo} alt="Dhako Logo" className="w-full h-full object-cover" />
          </div>
          <div className="text-center">
            <div className="font-bold text-sm tracking-widest text-white">DHAKO</div>
            ...
            return (
              <div key={item.page}>
                ...
                >
                  ...
                  {hasChildren && (
                    <span style={{ opacity: 0.5, fontSize: 9, transition: "transform 0.15s", transform: exp ? "rotate(0)" : "rotate(-90deg)", display: "inline-block" }}>▼</span>
                  )}
                </button>
                {hasChildren && exp && (
                  <div className="ml-8 mr-2 mb-1 pl-3" style={{ borderLeft: "1px solid rgba(255,255,255,0.2)" }}>
        ...
        <div className="px-4 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.12)" }}>
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs" style={{ color: "rgba(255,255,255,0.6)" }}>{user?.role.replace(/_/g, " ") || "Guest"}</div>
            </div>
          </div>
          <button
            onClick={onLogout}
            className="w-full px-3 py-2 text-sm font-semibold text-white rounded-lg transition-all hover:opacity-90"
