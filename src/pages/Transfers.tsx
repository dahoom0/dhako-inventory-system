...
const Transfers: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const toWarehouse = WAREHOUSES.find((w) => w.id === formData.toWarehouse);
      ...
      setSuccessMessage(
        `✓ Transfer created from ${fromWarehouse?.name} to ${toWarehouse?.name}`
      );
  ...
  const handleAdvanceStatus = async (transfer: Transfer, action: string) => {
    try {
      const newStatus =
        action === "APPROVE"
          ? "APPROVED"
      ...
      setSuccessMessage(`✓ Transfer ${action.toLowerCase()} successful`);
      setTimeout(() => setSuccessMessage(""), 3000);
    } catch (error) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {showForm && (
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Create New Transfer</h3>
