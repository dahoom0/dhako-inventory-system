...
import { BRANCH_PERF, EXPENSES, DEBTS, fmt } from "../../data/mock";
import { Card, PageHeader, Th, Td } from "../../components/ui";

export default function BranchReport() {
  const branchData = BRANCH_PERF.map(b => {
    const gross = b.sales - b.cogs;
    const net = gross - b.expenses;
    const margin = (gross / b.sales * 100).toFixed(1);
    const totalDebt = DEBTS.filter(d => d.branch === b.branch && d.status !== "Paid").reduce((s, d) => s + (d.original - d.paid), 0);
    return { ...b, gross, net, margin, totalDebt };
  });

  const winner = branchData.reduce((a, b) => (a.net > b.net ? a : b));
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="rounded-xl p-4 mb-5 flex items-center gap-4" style={{ background: "#dbeafe", border: "1px solid #bfdbfe" }}>
        ...
        <div>
          <div className="font-bold" style={{ color: "#1e3a8a" }}>{winner.branch} — Best Net Profit</div>
        ...
        {branchData.map(b => (
          <Card key={b.branch} className="p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <div className="font-bold" style={{ color: "#1e3a8a" }}>{b.branch}</div>
